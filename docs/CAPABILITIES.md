...
| Method | Endpoint | Description |
|--------|----------|-------------|
| POST | `/api/webhooks/trigger` | Inbound trigger (CI, Slack). Auth: `X-Webhook-Secret` or body.secret. Body: `{ action: 'ship'|'chat', params }`. Returns 202 + jobId. For chat, use POST `/api/chat/stream` directly; webhook `action: 'chat'` is out of scope and returns a message directing to `/api/chat/stream`. |
...
## Native Messaging (Telegram, Discord, WhatsApp, SMS)

G-Rump has **native** support for Telegram, Discord, WhatsApp (via Twilio), and SMS/voice (via Twilio). No Moltbot required. Users can chat, run SHIP (e.g. "ship Build a todo app"), and receive async notifications when SHIP completes.

| Platform | Endpoint | Config |
|----------|----------|--------|
| **Telegram** | `POST /api/messaging/telegram` | `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET` (optional). Set webhook: `https://api.telegram.org/bot{TOKEN}/setWebhook?url={PUBLIC_URL}/api/messaging/telegram` |
| **Discord** | Gateway (bot) | `DISCORD_BOT_TOKEN`. Requires non-serverless (Docker, Railway). Enable Message Content intent in Discord Developer Portal. |
| **Twilio (SMS/voice/WhatsApp)** | `POST /api/messaging/inbound` | `MESSAGING_PROVIDER=twilio`, `TWILIO_ACCOUNT_SID`, `TWILIO_AUTH_TOKEN`, `TWILIO_REPLY_TO_NUMBER`, `TWILIO_WEBHOOK_SECRET`. For WhatsApp: `TWILIO_WHATSAPP_NUMBER` or use same number with `whatsapp:` prefix. |

**Flow:** Inbound message → `messagingService.processMessage` → chat or ship intent → reply. Ship: start session, enqueue job, subscribe for `ship.completed` → notify user when done.

See [MESSAGING_SETUP.md](MESSAGING_SETUP.md) for setup instructions.
...
- **Moltbot / Clawdbot**: G-Rump has native Telegram, Discord, and WhatsApp (see above). The [integrations/moltbot-skill](../integrations/moltbot-skill) is optional for users who prefer Moltbot's deployment model. Config: `GRUMP_API_URL`, `GRUMP_API_KEY`.
