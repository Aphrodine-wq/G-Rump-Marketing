...
3. Point users to your web URL and have them sign in (example: `https://app.g-rump.com`). For multi-instance and Redis session storage, see [SCALING.md](SCALING.md).
...
- **Tray**: Closing the window hides the app to the system tray; use **Quit** from the tray menu to exit.
- **Shortcut**: Press **Ctrl+Shift+G** from anywhere to show and focus G-Rump.
- **Tray menu**: New chat, Settings, Open workspace folder (opens the current workspace in Explorer).
- **Protocol**: In a packaged build, `grump://chat` and `grump://settings` open the app and navigate (e.g. from a browser or link).
...
| `SUPABASE_URL` | No | - | Supabase URL (optional; backend uses mock auth if not set) |
| `SUPABASE_ANON_KEY` | No | - | Supabase anon key (required for real auth when SUPABASE_URL is set) |
| `SUPABASE_SERVICE_KEY` | No | - | Supabase service key (optional, for admin operations) |
| `STRIPE_SECRET_KEY` / `STRIPE_WEBHOOK_SECRET` | No | - | Stripe billing (optional; see [PRODUCTION_CHECKLIST](PRODUCTION_CHECKLIST.md#web-app-supabase-and-stripe-mock-vs-real)) |
