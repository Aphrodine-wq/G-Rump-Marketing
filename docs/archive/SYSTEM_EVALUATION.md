...
**See [SYSTEM_RATING.md](SYSTEM_RATING.md) for current score and remaining tasks.**
...
| **Production readiness** | **~50%** | [PRODUCTION_CHECKLIST](PRODUCTION_CHECKLIST.md) and observability in place; completion % reflects items still to be configured and verified per checklist. |
...
- **Webhook chat trigger:** “Out of scope; use POST `/api/chat/stream` for chat. `POST /api/webhooks/trigger` with `action: 'chat'` returns 202 and a message directing to `/api/chat/stream`.”
...
- **Webhook chat trigger:** Out of scope; use POST `/api/chat/stream`. Implement webhook-triggered chat only if needed.
